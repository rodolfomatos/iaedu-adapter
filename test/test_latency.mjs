...
if (!API_KEY) {
  console.error(
    'ERRO: Define a variável de ambiente IAEDU_API_KEY antes de correr.'
  );
  process.exit(1);
}
...
async function sendRequest(testName, message, threadId) {
  ...
  try {
    ...
    console.log(
      `Tempo até ao primeiro byte (TTFB): ${(ttfb / 1000).toFixed(2)} segundos`
    );
    ...
    console.log(
      ...
      decoder.decode(value).substring(0, 50) + '...'
    );
...
async function runTests() {
  ...
  await sendRequest(
    'Ping Simples',
    'Olá, isto é um teste de latência.',
    randomId
  );

  // Teste 2: Mensagem um pouco maior (simulação básica)
  const longMessage =
    'Explica detalhadamente a teoria da relatividade geral e como ela se compara com a mecânica quântica, fornecendo exemplos históricos.';
