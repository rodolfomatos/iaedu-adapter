...
async function testSpeed() {
  ...
  const message =
    'Escreve um poema longo sobre a importância da velocidade na computação.';
  ...
  try {
    ...
    while (true) {
      ...
      if (firstTokenTime === 0) {
        ...
        console.log(
          `Tempo até ao primeiro token (Latência): ${((firstTokenTime - startTotal) / 1000).toFixed(2)}s`
        );
      }
    ...
    if (tps < 5)
      console.warn(
        'AVISO: A API de origem é MUITO LENTA. O adaptador não pode fazer milagres.'
      );
